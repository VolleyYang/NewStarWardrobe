...
public class ClassifyGridAdapter extends BaseAdapter{
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        holder.mTextView.setText(data.get(0).getData().getItems().get(type).getComponent().getItems().get(position).getComponent().getWord());
        Glide.with(mContext).load(data.get(0).getData().getItems().get(type).getComponent().getItems().get(position).getComponent().getPicUrl()).into(holder.mImageView);
